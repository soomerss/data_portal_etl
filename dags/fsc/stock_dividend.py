...
def get_daily_stock_data(date):
    ...
    df = pd.DataFrame()
    while True:
        url = f"http://apis.data.go.kr/1160100/service/GetStocDiviInfoService/getDiviInfo?serviceKey={key}&basDt={date}&resultType=json&numOfRows=10000&pageNo={page}"
        req = requests.get(url)
        json_file = json.loads(req.text)
        req_df = pd.json_normalize(json_file['response']['body']['items']['item'])
        if req_df.empty :
            break
        else :
            df = pd.concat([df,req_df],ignore_index=True)
            page += 1
    print(date)
    print(df.head(1))
    return df
...
if __name__=="__main__":
    ...
    final_data.to_csv(f"stock_dividend-from-{str(year)+str(month).zfill(2)+str(day).zfill(2)}-to-{date.today().strftime('%Y%m%d')}",index=False)
    end = time.time() - start
    print(end)
