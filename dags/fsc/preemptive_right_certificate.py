...
def get_daily_stock_data(date):
    with open('key.json','r') as f:
        ...
        key = json_data['key']
    url = f"https://apis.data.go.kr/1160100/service/GetStockSecuritiesInfoService/getPreemptiveRightCertificatePriceInfo?serviceKey={key}&basDt={date}&resultType=json&numOfRows=10000&pageNo=1"
...
if __name__=="__main__":
    ...
    final_data.to_csv(f"preemtive_right_certificate-from-{str(year)+str(month).zfill(2)+str(day).zfill(2)}-to-{date.today().strftime('%Y%m%d')}",index=False)
    end = time.time() - start
    print(end)